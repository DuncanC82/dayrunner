...
type Staff = { id: string; name: string; licence_class: number; p_endorsement: boolean; max_hours: number; skills: string[]; notes: string | null; active: boolean };
...
type Product = { id: string; name: string; duration_minutes: number; skills_required: string[]; needs_separate_guide: boolean; pickup_lead_minutes: number };
type Departure = { id: string; product_id: string | null; product_name: string; date: string; time: string };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const breakMin = Number(rule.break_minutes ?? 30);
    ...
    // ----- state -----
    const hoursUsed = new Map<string, number>(); const busyUntil = new Map<string, number>(); const vehBusyUntil = new Map<string, number>();
    ...
    for (const { d, p, bk } of work) {
      ...
      const notes: string[] = []; let status: "ok" | "warn" | "bad" = "ok";
      ...
      if (chosen.length === 0) {
        ...
        for (const v of chosen) { vehBusyUntil.set(v.id, end + 15); if (v.status === "warning") { status = status === "bad" ? "bad" : "warn"; notes.push(`${v.name} has a warning: ${v.notes ?? "check before departure"}.`); } }
      ...
      const driverOk = (s: Staff, v?: Vehicle) => {
        ...
        if (a?.available_from && mins(a.available_from) > start) return false;
        if (a?.available_to && mins(a.available_to) < end) return false;
        if ((busyUntil.get(s.id) ?? -1) > start) return false;
      ...
      for (const v of chosen) {
        ...
        drivers.push(pick);
        const hrs = (dur + breakMin * Math.max(1, Math.floor(dur / 300))) / 60; const used = (hoursUsed.get(pick.id) ?? 0) + hrs;
        hoursUsed.set(pick.id, used); busyUntil.set(pick.id, end + 15);
        if (used > pick.max_hours) { status = status === "bad" ? "bad" : "warn"; exceptions.push({ level: "bad", title: `${pick.name} exceeds ${pick.max_hours}h on ${d.product_name}`, detail: `${d.product_name} is ${(dur / 60).toFixed(1)}h door to door plus breaks; ${pick.name} would be at ${used.toFixed(1)}h. Fatigue rules and the licence hours cap apply.`, options: ["Pair with a second driver for the return leg", "Swap for a driver with more hours", "Shorten the itinerary"] }); driverNotes.push(`${pick.name} over hours (${used.toFixed(1)}h).`); }
      ...
      if (p?.needs_separate_guide) { guide = S.find((s) => !drivers.includes(s) && driverOk(s) && skillsReq.every((k) => s.skills.map(norm).includes(k))) ?? null; if (!guide) { status = status === "bad" ? "bad" : "warn"; driverNotes.push("No separate guide available; driver guides."); } else busyUntil.set(guide.id, end + 15); }
    ...
    const summary = { departures: deps.length, pax: bookings.reduce((a, b) => a + b.pax, 0), messages: messages.length, exceptions: exceptions.filter((e) => e.level === "bad").length, warnings: exceptions.filter((e) => e.level === "warn").length, narrative: (globalThis as any).__summary ?? null };
