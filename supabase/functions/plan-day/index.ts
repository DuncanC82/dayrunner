  ...
  try {
    ...
    if (key && messages.length) {
      try {
        ...
        const r = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "x-api-key": key, "anthropic-version": "2023-06-01", "content-type": "application/json", ...(Deno.env.get("ANTHROPIC_WORKSPACE_ID") ? { "anthropic-workspace-id": Deno.env.get("ANTHROPIC_WORKSPACE_ID")! } : {}) }, body: JSON.stringify({ model: "claude-sonnet-5", max_tokens: 12000, messages: [{ role: "user", content: prompt }] }) });
        if (r.ok) { const out = await r.json(); const text = (out.content ?? []).filter((b: any) => b.type === "text").map((b: any) => b.text).join("\n"); let j: any; try { j = JSON.parse(text.slice(text.indexOf("{"), text.lastIndexOf("}") + 1)); } catch (pe) { throw new Error(`parse failed (stop=${out.stop_reason}, len=${text.length}): ${text.slice(0, 200)}`); } for (const m of j.messages ?? []) if (messages[m.i] && m.body) messages[m.i].body = m.body; generated_by = "allocator+claude"; (globalThis as any).__summary = j.summary; }
        else { const t = await r.text(); console.error("claude pass failed", r.status, t.slice(0, 300)); (globalThis as any).__claude_error = `Claude ${r.status}: ${t.slice(0, 200)}`; }
      } catch (e) { console.error("claude pass error", e); (globalThis as any).__claude_error = String((e as any)?.message ?? e); /* keep deterministic output */ }
    }

    // ----- persist -----
    await db.from("plans").delete().eq("operator_id", operator_id).eq("date", date);
    const summary = { departures: deps.length, pax: bookings.reduce((a, b) => a + b.pax, 0), km: Math.round([...kmBy.values()].reduce((a, b) => a + b, 0)), work_hours: Math.round([...ledgers.values()].reduce((a, l) => a + l.dayMinutes, 0) / 6) / 10, worktime_regime: regime, ops_mode: charter ? "charter" : "fleet", transport_request_id: tr?.id ?? null, messages: messages.length, exceptions: exceptions.filter((e) => e.level === "bad").length, warnings: exceptions.filter((e) => e.level === "warn").length, narrative: (globalThis as any).__summary ?? null, claude_error: (globalThis as any).__claude_error ?? null };
