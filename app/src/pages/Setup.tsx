...
export default function Setup() {
  ...
  return (
    <div>
      ...
      <Table title="Driver-guides" rows={staff} blank={{ licence_class: 1, max_hours: 10, p_endorsement: true }} cols={[{ key: "name", label: "Name" }, { key: "phone", label: "Phone" }, { key: "licence_class", label: "Licence class", type: "number" }, { key: "max_hours", label: "Max hours", type: "number" }, { key: "skills", label: "Skills (comma)", placeholder: "milford, wine" }, { key: "notes", label: "Notes" }]}
        onAdd={async (n) => { await supabase.from("staff").insert({ operator_id: oid, name: n.name, phone: n.phone || null, licence_class: Number(n.licence_class || 1), max_hours: Number(n.max_hours || 10), skills: list(n.skills), notes: n.notes || null }); await load(); }} onDelete={del("staff")} />
      ...
      <Table title="Products" rows={products} blank={{ duration_minutes: 240, pickup_lead_minutes: 25 }} cols={[{ key: "name", label: "Name (exactly as the booking system sends it)" }, { key: "duration_minutes", label: "Duration (min)", type: "number" }, { key: "skills_required", label: "Skills needed (comma)" }, { key: "pickup_lead_minutes", label: "Pickup lead (min)", type: "number" }]}
        onAdd={async (n) => { await supabase.from("products").insert({ operator_id: oid, name: n.name, duration_minutes: Number(n.duration_minutes || 240), skills_required: list(n.skills_required), pickup_lead_minutes: Number(n.pickup_lead_minutes || 25) }); await load(); }} onDelete={del("products")} />

      <Table title="Suppliers" rows={suppliers} blank={{ channel: "email", detail_template: "Numbers for {date}: {pax} pax", confirm_by: "17:00 day before" }} cols={[{ key: "name", label: "Supplier" }, { key: "contact", label: "Contact" }, { key: "product_names", label: "Products (comma, partial ok)", placeholder: "Milford" }, { key: "detail_template", label: "What to confirm ({date} {pax} {product} {time})" }, { key: "confirm_by", label: "Confirm by" }]}
        onAdd={async (n) => { await supabase.from("suppliers").insert({ operator_id: oid, name: n.name, contact: n.contact || null, product_names: list(n.product_names), detail_template: n.detail_template || "Numbers for {date}: {pax} pax", confirm_by: n.confirm_by || "17:00 day before" }); await load(); }} onDelete={del("suppliers")} />
    </div>
  );
}
